...
This section introduces the `ams_ami` data resource to query for a machine
image to deploy with the ec2 instance.

This also introduced the `filter` metaparameter as well that is used to narrow
down the results returns from a data resource.

Additionally, the `aws_key_pair` resource was introduced as well.  This is to
ensure you have a key credential to log in with since Terraform is unable to
retrieve the key generated by AWS like when you deploy an instance interactively.

Note that the `variables.tf` file is not pointing to the default public key
file name that would be in `~/.ssh/` but instead a custom key was made for the
